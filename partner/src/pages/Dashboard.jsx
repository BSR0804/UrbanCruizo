...
import {
    LayoutDashboard, Car, CalendarCheck, Wallet, Bell, Settings, Plus, TrendingUp, Users, MapPin, Trash2, Edit, Check, X, Eye, MessageSquare, RefreshCw, Lock, ChevronRight
} from 'lucide-react';
...
const Dashboard = () => {
    ...
    const demoStats = {
        totalVehicles: 12, activeVehicles: 9, bookedVehicles: 3, totalBookings: 7, pendingBookings: 2,
        totalEarnings: 184500, recentActivity: [
            { vehicle: { title: 'Mercedes-Benz C-Class' }, user: { name: 'Rahul Sharma' }, createdAt: new Date().toISOString() },
            { vehicle: { title: 'BMW X5 M-Sport' }, user: { name: 'Priya Patel' }, createdAt: new Date(Date.now() - 3600000).toISOString() },
            { vehicle: { title: 'Hyundai Creta' }, bookingName: 'Amit Kumar', createdAt: new Date(Date.now() - 7200000).toISOString() }
        ]
    };
    const demoVehicles = [
        { _id: '1', title: 'Mercedes-Benz C-Class', brand: 'Mercedes', model: 'C-Class', pricePerDay: 8500, location: 'Gachibowli', city: 'Hyderabad', availability: true, images: [] },
        { _id: '2', title: 'BMW X5 M-Sport', brand: 'BMW', model: 'X5', pricePerDay: 12000, location: 'Jubilee Hills', city: 'Hyderabad', availability: false, images: [] },
        { _id: '3', title: 'Hyundai Creta', brand: 'Hyundai', model: 'Creta', pricePerDay: 3500, location: 'Banjara Hills', city: 'Hyderabad', availability: true, images: [] },
        { _id: '4', title: 'Kia Seltos', brand: 'Kia', model: 'Seltos', pricePerDay: 4000, location: 'Madhapur', city: 'Hyderabad', availability: true, images: [] },
    ];
    const demoBookings = [
    ...
    const handleVehicleSubmit = async (e) => {
        e.preventDefault();
        if (!isAuthenticated) { toast.error('Please login to manage vehicles'); return; }
        try {
            const imagesArray = typeof vehicleFormData.images === 'string' ? vehicleFormData.images.split(',').map(img => img.trim()) : vehicleFormData.images;
            const dataToSubmit = { ...vehicleFormData, images: imagesArray };
            if (editingVehicle) { await axios.put(`vehicles/${editingVehicle._id}`, dataToSubmit); toast.success('Vehicle updated!'); }
            else { await axios.post('vehicles', dataToSubmit); toast.success('Vehicle added!'); }
            setShowAddModal(false); setEditingVehicle(null); fetchData();
    ...
    const handleDeleteVehicle = async (id) => {
        ...
        if (window.confirm('Delete this vehicle permanently?')) {
            try { await axios.delete(`vehicles/${id}`); toast.success('Vehicle removed'); fetchData(); }
            ...
            <div className="container mx-auto max-w-7xl">
                ...
                <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
                    <div>
                        ...
                        <h1 className="text-4xl font-serif font-bold text-white mb-2 underline decoration-primary/50 underline-offset-8">Partner Dashboard</h1>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            <div className="text-right hidden sm:block">
                                <p className="text-sm font-bold text-white leading-none">{displayUser?.name || 'Partner'}</p>
                                <p className="text-[10px] text-primary uppercase tracking-widest mt-1">{displayUser?.businessName || (isAuthenticated ? 'Authorized Dealer' : 'Demo Account')}</p>
                            </div>
                            <div className="w-10 h-10 bg-primary rounded-full flex items-center justify-center font-bold text-background shadow-lg shadow-primary/20">{displayUser?.name ? displayUser.name[0] : 'P'}</div>
                        </div>
                    </div>
                </header>
                        ...
                        <AnimatePresence mode="wait">
                            ...
                            {activeTab === 'overview' && (
                                <motion.div key="overview" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
                                    <div className="relative p-10 rounded-[3rem] bg-gradient-to-br from-primary to-yellow-700 overflow-hidden shadow-2xl shadow-primary/20">
                                        <div className="absolute -top-20 -right-20 w-80 h-80 bg-white/10 rounded-full blur-[80px]" />
                                        <div className="relative z-10">
                                            <h2 className="text-3xl md:text-5xl font-serif font-black text-background mb-4">Welcome back, <span className="italic">{displayUser?.name?.split(' ')[0] || 'Partner'}</span></h2>
                                            <p className="text-background/80 max-w-lg leading-relaxed font-medium">Your fleet is performing exceptionally well. You've earned ₹{stats?.totalEarnings?.toLocaleString()} after commission.</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid md:grid-cols-2 gap-8">
                                        ...
                                        <div className="bg-background border border-gray-800 p-8 rounded-[2.5rem] flex flex-col justify-center">
                                            ...
                                            <div className="w-full h-3 bg-surface rounded-full overflow-hidden">
                                                <motion.div initial={{ width: 0 }} animate={{ width: `${stats?.totalVehicles > 0 ? (stats?.activeVehicles / stats?.totalVehicles * 100) : 0}%` }} className="h-full bg-primary rounded-full" />
                                            </div>
                                            <p className="text-[10px] text-textSecondary mt-4 italic">Based on vehicle availability vs total listings.</p>
                                        </div>
                                    </div>
                                        ...
                                        <div className="space-y-6">
                                            {stats?.recentActivity?.length > 0 ? stats.recentActivity.map((activity, idx) => (
                                                <div key={idx} className="flex gap-4 items-start group">
                                                    <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center shrink-0 text-primary"><Bell className="w-4 h-4" /></div>
                                                    <div>
                                                        <p className="text-white text-sm">New booking request for <span className="font-bold text-primary">{activity.vehicle?.title}</span> by <span className="text-gray-300">{activity.user?.name || activity.bookingName}</span></p>
                            ...
                            {activeTab === 'vehicles' && (
                                <motion.div key="vehicles" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
                                    ...
                                    <div className="grid md:grid-cols-2 gap-6">
                                        {vehicles.filter(v => v.title.toLowerCase().includes(searchTerm.toLowerCase()) || v.city?.toLowerCase().includes(searchTerm.toLowerCase()) || v.location?.toLowerCase().includes(searchTerm.toLowerCase())).map(vehicle => (
                                            <div key={vehicle._id} className="bg-background border border-gray-800 rounded-3xl p-6 group">
                                                <div className="relative h-48 mb-6 rounded-2xl overflow-hidden flex items-center justify-center bg-surface">
                                                    {vehicle.images?.[0] ? (<img src={vehicle.images[0]} alt={vehicle.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />) : (<Car className="w-16 h-16 text-gray-700" />)}
                                                    <div className={`absolute top-4 right-4 backdrop-blur-md px-3 py-1 rounded-full text-[10px] font-bold uppercase border ${vehicle.availability ? 'bg-green-500/20 text-green-400 border-green-500/20' : 'bg-red-500/20 text-red-400 border-red-500/20'}`}>{vehicle.availability ? 'Available' : 'Booked'}</div>
                                                </div>
                                                <h3 className="text-xl font-bold text-white mb-1">{vehicle.title}</h3>
                                                <p className="text-sm text-textSecondary flex items-center gap-2 mb-6"><MapPin className="w-4 h-4 text-primary/60" /> {vehicle.location}, {vehicle.city}</p>
                                                <div className="flex justify-between items-center bg-surface/50 p-4 rounded-xl border border-gray-800/50">
                                                    ...
                                                    <div className="flex gap-2">
                                                        <button onClick={() => { if (!isAuthenticated) { toast.error('Login required'); return; } setEditingVehicle(vehicle); setVehicleFormData({ ...vehicle, images: vehicle.images?.join(', ') || '' }); setShowAddModal(true); }} className="p-2 text-blue-500 hover:bg-blue-500/10 rounded-lg transition-colors" title="Edit"><Edit className="w-4 h-4" /></button>
                                                        <button onClick={async () => { if (!isAuthenticated) { toast.error('Login required'); return; } try { await axios.put(`vehicles/${vehicle._id}`, { availability: !vehicle.availability }); toast.success(`Status updated`); fetchData(); } catch (e) { toast.error('Failed'); } }} className={`p-2 rounded-lg transition-colors ${vehicle.availability ? 'text-green-500 hover:bg-green-500/10' : 'text-orange-500 hover:bg-orange-500/10'}`} title="Toggle"><Eye className="w-4 h-4" /></button>
                                                        <button onClick={() => handleDeleteVehicle(vehicle._id)} className="p-2 text-red-500 hover:bg-red-500/10 rounded-lg transition-colors" title="Delete"><Trash2 className="w-4 h-4" /></button>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                                    ...
                                    <div className="space-y-6">
                                        {bookings.length > 0 ? bookings.map(booking => (
                                            <div key={booking._id} className="bg-background border border-gray-800 rounded-3xl p-6 flex flex-col md:flex-row gap-6 relative overflow-hidden group">
                                                ...
                                                <div className="w-full md:w-32 h-32 bg-surface rounded-2xl shrink-0 flex items-center justify-center overflow-hidden border border-gray-800">{booking.vehicle?.images?.[0] ? (<img src={booking.vehicle.images[0]} className="w-full h-full object-cover" />) : <Car className="w-10 h-10 text-gray-700" />}</div>
                                            ...
                                            <div className="text-center py-20 bg-background/50 rounded-3xl border border-dashed border-gray-800"><CalendarCheck className="w-12 h-12 text-gray-800 mx-auto mb-4" /><p className="text-textSecondary italic">No booking requests yet.</p></div>
                                        )}
                                    </div>
                                </motion.div>
                            )}
                            ...
                            {activeTab === 'requests' && (
                                <motion.div key="requests" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
                                    ...
                                    <p className="text-sm text-textSecondary italic bg-primary/5 p-4 rounded-xl border border-primary/10">These are custom requests from users. Reach out to them to strike a deal!</p>
                                    <div className="space-y-6">
                                        {carRequests.length > 0 ? carRequests.map(req => (
                                            req && (
                                                <div key={req._id} className={`bg-background border border-gray-800 rounded-3xl p-6 relative group border-l-4 transition-all ${req.city?.toLowerCase() === displayUser?.city?.toLowerCase() ? 'border-l-primary shadow-xl shadow-primary/5' : 'border-l-gray-800 opacity-80 hover:opacity-100'}`}>
                                                    {req.city?.toLowerCase() === displayUser?.city?.toLowerCase() && (<div className="absolute top-4 right-16 bg-primary/20 text-primary border border-primary/20 px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest italic animate-pulse">City Match</div>)}
                                                    <div className="flex justify-between items-start mb-4">
                                                        <div><h3 className="text-lg font-serif font-black text-white italic">{req.requirements}</h3><p className="text-[10px] text-primary uppercase tracking-widest font-black mt-1 italic">Type: {req.vehicleType}</p></div>
                                                    ...
                                                    <button onClick={() => window.open(`tel:${req.phone}`)} className="w-full mt-6 py-3 bg-surface hover:bg-primary hover:text-background transition-all rounded-xl text-[10px] font-black uppercase tracking-[0.3em] flex items-center justify-center gap-2">Contact Lead <MessageSquare className="w-4 h-4" /></button>
                                                </div>
                                            )
                                        )) : (<div className="text-center py-20 bg-background/50 rounded-3xl border border-dashed border-gray-800"><MessageSquare className="w-12 h-12 text-gray-800 mx-auto mb-4" /><p className="text-textSecondary italic">No general requests at the moment.</p></div>)}
                                    </div>
                                </motion.div>
                            )}
                            ...
                            {activeTab === 'earnings' && (
                                <motion.div key="earnings" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
                                    ...
                                    <div className="bg-background border border-gray-800 rounded-[2.5rem] overflow-hidden">
                                        <div className="p-8 border-b border-gray-800"><h3 className="text-xl font-bold text-white">Earnings History</h3><p className="text-xs text-textSecondary mt-1 uppercase tracking-widest font-black italic">Recent Confirmed Bookings</p></div>
                                        <div className="overflow-x-auto">
                                            <table className="w-full text-left">
                                                <thead className="bg-surface/50 text-[10px] uppercase font-black tracking-[0.2em] text-textSecondary"><tr><th className="px-8 py-4">Ref ID</th><th className="px-8 py-4">Date</th><th className="px-8 py-4">Vehicle</th><th className="px-8 py-4">Net Amount</th><th className="px-8 py-4">Status</th></tr></thead>
                                                <tbody className="divide-y divide-gray-800/50">
                                                    {bookings.filter(b => b.status === 'approved' || b.status === 'confirmed').length > 0 ? (
                                                        ...
                                                        <tr><td colSpan="5" className="px-8 py-20 text-center text-textSecondary italic">No earnings records found yet.</td></tr>
                                                    )}
                                                </tbody>
                                            </table>
                                        </div>
                                    ...
                                    <div className="bg-background border border-gray-800 p-8 rounded-[2rem] space-y-6">
                                        <div className="space-y-2">
                                            ...
                                            <div className="flex items-center gap-4 bg-surface p-4 rounded-2xl border border-gray-800">
                                                <div className="w-12 h-12 bg-primary/20 rounded-full flex items-center justify-center text-primary font-bold">{displayUser?.name ? displayUser.name[0] : 'P'}</div>
                                                <div><p className="text-white font-bold">{displayUser?.name || 'Partner'}</p><p className="text-xs text-textSecondary">{displayUser?.email || 'demo@urbancruizo.com'}</p></div>
                                            </div>
                                        </div>
                                        <button onClick={() => { if (!isAuthenticated) { toast.error('Login required'); return; } setShowProfileForm(true); }} className="w-full btn-outline py-4 rounded-xl flex items-center justify-center gap-2">Update Profile Details <Edit className="w-4 h-4" /></button>
                                    </div>
                                </motion.div>
                            )}
                        </AnimatePresence>
